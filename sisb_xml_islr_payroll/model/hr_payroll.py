...
class hr_payslip(osv.osv):
	...
	def _get_withholding_fnc(self, cr, uid, ids, field_names, arg=None, context=None):
		res = {}
		for hr_payslip_obj in self.browse(cr, uid, ids, context=context):
	...
	def _get_ret_isrl_fnc(self, cr, uid, ids, field_names, arg=None, context=None):
		hr_payslip_line_obj = self.pool.get('hr.payslip.line')
		res = {}
		for hr_payslip in self.browse(cr, uid, ids, context=context):
			hr_payslip_line_ids = hr_payslip_line_obj.search(cr, uid, [('slip_id', '=', hr_payslip.id),('code','=','039')], context=context)

			if hr_payslip_line_ids:
				
				hr_payslip_bwr = hr_payslip_line_obj.browse(cr, uid, hr_payslip_line_ids, context=context)
	...
	_columns = {
		...
		'rate_id': fields.many2one(
            ...
            help="Person type"),
		'partner_vat': fields.function(_get_vat_fnc, type="char", string='VAT'),
		'total_amount' : fields.function(_get_ret_isrl_fnc, type="float", string='Total amount'),
		'withholding' : fields.function(_get_withholding_fnc, type="float", string='Tasa Ret ISRl'),
	}
