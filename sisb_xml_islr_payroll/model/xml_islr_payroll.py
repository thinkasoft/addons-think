...
# -*- encoding: utf-8 -*-
###########################################################################
#    Copyright (C) Industrias Diana, C.A. (www.industriasdiana.gob.ve)
#    All Rights Reserved
###############Credits######################################################
#    Developed by: Industrias Diana, C.A.
##############################################################################
...
class islr_xml_wh_doc(osv.osv):
    ...
    _defaults = {
        'name':lambda self,cr,uid,context : _('Withholding Income ')+time.strftime('%m/%Y')
    }

    def _get_amount_total_payroll(self,cr,uid,ids,name,args,context=None):
        res = {}
    ...
    def _get_amount_total_base_payroll(self,cr,uid,ids,name,args,context=None):
        res = {}
        for xml in self.browse(cr,uid,ids,context):
            res[xml.id]= 0.0
            for line in xml.xml_payroll_ids:
    ...
    _columns = {
        'xml_ids':fields.one2many('islr.xml.wh.line','islr_xml_wh_doc','XML Document Lines'),
        'xml_payroll_ids': fields.one2many('hr.payslip', 'islr_xml_wh_line_payroll_id', 'XML Document Payroll Lines'),
        'amount_total_ret_payroll':fields.function(_get_amount_total_payroll,method=True, digits=(16, 2), readonly=True, string='Withholding Income Amount Total for payroll', help="Amount Total of withholding - Payroll"),
        'amount_total_base_payroll':fields.function(_get_amount_total_base_payroll,method=True, digits=(16, 2), readonly=True, string='Without Tax Amount Total for payroll', help="Total without taxes - Payroll"),
    }
    ...
    def action_confirm2(self, cr, uid, ids, context={}):
        for xml_doc_brw in self.browse(cr, uid, ids, context=context):
            if not xml_doc_brw.xml_ids and not xml_doc_brw.xml_payroll_ids:
                raise osv.except_osv(_('Error!'), _("Retentions must be loaded before confirming"))
                
        return super(islr_xml_wh_doc, self).action_confirm1(cr, uid, ids, context)
    ...
    def _xml2(self, cr,uid,ids):
        ...
        for id in ids:
            ...
            fecha_fin = datetime.datetime.strptime(wh_brw.period_id.date_stop,'%Y-%m-%d')
            #######################################################
            ...
            if wh_brw.xml_ids:
                for line in xml_lines:
                    ...
                    SubElement(detalle, "FechaOperacion").text = fecha_contable_ai
            ...
            for line_payroll in wh_brw.xml_payroll_ids:
                detalle = SubElement(root,"DetalleRetencion")
                SubElement(detalle, "RifRetenido").text = line_payroll.partner_vat
                SubElement(detalle, "NumeroFactura").text = str(0)
                SubElement(detalle, "NumeroControl").text = str(0)
                SubElement(detalle, "FechaOperacion").text = fecha_fin.strftime('%d/%m/%Y')
                SubElement(detalle, "CodigoConcepto").text = '001'
                SubElement(detalle, "MontoOperacion").text = str(line_payroll.total_amount)
                SubElement(detalle, "PorcentajeRetencion").text = str(line_payroll.withholding)
                #SubElement(detalle, "PorcentajeRetencion").text = str(line_payroll.porcent_rete)
